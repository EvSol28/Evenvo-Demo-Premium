...
try {
  ...
  if (process.env.NODE_ENV === 'production' || process.env.FIREBASE_PRIVATE_KEY) {
    ...
    console.log('Clé privée (premiers 50 caractères):', process.env.FIREBASE_PRIVATE_KEY ? process.env.FIREBASE_PRIVATE_KEY.substring(0, 50) + '...' : 'undefined');
  ...
  console.log('Le serveur continuera sans Firebase - certaines fonctionnalités seront limitées');
}
...
try {
  ...
  console.error('Erreur lors de la configuration Firestore:', error.message);
  // Créer un mock firestore pour éviter les erreurs
  firestore = {
    collection: () => ({
...
app.get('/event/:eventId/suivi_vote/:formId', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (!formSnapshot.exists) {
            ...
            return res.status(404).send('Formulaire de vote introuvable.');
        }
        const formData = formSnapshot.data();

        // Vérifier que le formulaire appartient à cet événement
        if (formData.eventId !== eventId) {
            return res.status(403).send('Ce formulaire n\'appartient pas à cet événement.');
        }
